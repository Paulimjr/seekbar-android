...
package com.seekbar.rangeseekbar;
...
import com.seekbar.R;
import com.seekbar.util.BitmapUtil;
import com.seekbar.util.PixelUtil;
...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultShadowColor = Color.argb(0, 255, 255, 255);
        ...
        if (attrs == null) {
            ...
            defaultColor = Color.argb(20, 255,255,255);
            alwaysActive = false;
            ...
            try {
                ...
                defaultColor = a.getColor(R.styleable.RangeSeekBar_defaultColor, Color.argb(20, 255,255,255));
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (notifyWhileDragging && listener != null) {
                        ...
                        int x = (int) ((getSelectedMinValue().intValue()+increment)/increment);
                        x = x*increment;
                        if (x<absoluteMaxValue.intValue()){

                            minText = x ;
                            listener.onRangeSeekBarValuesChanged(this, getSelectedMinValue(), getSelectedMaxValue(),maxText, minText);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (listener != null) {
                    int minText;
                    int maxText = getSelectedMaxValue().intValue();

                    int x =  ((getSelectedMinValue().intValue()+increment)/increment);
                    x = x*increment;
                    if (x<absoluteMaxValue.intValue()){
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            String minText = valueToString(getSelectedMinValue())+ mStringValue;
            String maxText = valueToString(getSelectedMaxValue()) + mStringValue;

            int x = (int) ((getSelectedMinValue().intValue()+increment)/increment);
            x = x*increment;
            if (x<absoluteMaxValue.intValue())
                minText = x + mStringValue;
            else
                minText=(absoluteMaxValue.intValue()-increment) + mStringValue;


            x = (int) ((getSelectedMaxValue().intValue()+increment)/increment);
            x = x*increment;
            maxText = x + mStringValue;
            ...
            float minPosition = Math.max(0f, normalizedToScreen(normalizedMinValue) - minTextWidth * 1f);
            float maxPosition = Math.min(getWidth() - maxTextWidth, normalizedToScreen(normalizedMaxValue) - maxTextWidth * 1f);
    ...
    protected Parcelable onSaveInstanceState() {
        ...
        bundle.putDouble("", normalizedMinValue);
        bundle.putDouble("", normalizedMaxValue);
        return bundle;
    }
    ...
    public interface OnRangeSeekBarChangeListener<T extends Number> {
        ...
        void onRangeSeekBarValuesChanged(RangeSeekBar<T> bar, T minValue, T maxValue, int max, int min);
    }
